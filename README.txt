...
In this case, use -sep -o frame.pdb and convert the .pdb files in .gro files.
...
Parameters to set (lines 8-12 in the script):
...
Launch (Approximatively: 2h05min for 5 replicas launched in parallel - 5 replicas * 7500 frames - 6 threads - box size: 44x44x24 nm^3):
./command_fatslim.sh
...
The user needs to give one variable (line 11):
...
and check if all the lipids of the membrane studied are indicated in the lipid dictionary (line 12), if it is not the case, add it/them:
...
Launch: (Approximatively: 3h15min for 5 replicas launched in parallel - 5 replicas * 7500 frames - 6 threads - box size: 44x44x24 nm^3):
...
- upper_heatmap.pdf, upper_heatmap.svg, lower_heatmap.pdf, lower_heatmap.svg : depth maps for the upper and lower leaflets in .pdf and .svg, respectively.
- upper_contour.pdf, upper_contour.svg, lower_contour.pdf, lower_contour.svg : contour maps for the upper and lower leaflets in .pdf and .svg, respectively.
